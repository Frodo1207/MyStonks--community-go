...
CREATE TABLE `events` (
                          ...
                          `name` VARCHAR(128) NOT NULL COMMENT '活动名称(内部标识，英文或拼音)',
                          `description` TEXT COMMENT '活动详细描述',
                          `content` LONGTEXT COMMENT '活动富文本内容',
                          `cover_image_url` VARCHAR(255) COMMENT '封面图片链接',
                          `location` VARCHAR(128) COMMENT '活动地点(如"Twitter Space"、"Telegram群"、"北京798艺术区")',
                          `start_time` DATETIME NOT NULL COMMENT '活动开始时间',
                          `end_time` DATETIME NOT NULL COMMENT '活动结束时间',
                          `register_url` VARCHAR(255) COMMENT '参与/报名链接',
                          `max_attendees` INT NOT NULL DEFAULT 0 COMMENT '最大参与人数(0表示不限)',
                          `current_attendees` INT NOT NULL DEFAULT 0 COMMENT '当前参与人数',
                          `popularity` INT NOT NULL DEFAULT 0 COMMENT '热度(可根据浏览量、参与数等计算)',
                          `view_count` INT NOT NULL DEFAULT 0 COMMENT '浏览量',
                          `category_id` VARCHAR(64) COMMENT '分类ID',
                          `location_type` VARCHAR(32) COMMENT '地点类型(online/offline)',
